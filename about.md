---
layout: page
title: About
---

## whoami

Profesional de ciberseguridad ofensiva con 3 años de formación práctica intensiva. 
Apasionado del hacking ético, el Red Team y la resolución de máquinas CTF.

## Certificaciones

- **CPTS** — Certified Penetration Testing Specialist (HackTheBox)
- **OSCP** — En progreso (60/100 en primer intento)

  ## Plataformas

- **HackTheBox** — +100 máquinas resueltas con writeups
- **OffSec Labs** — +80 máquinas resueltas

## Writeups

Iré publicando progresivamente los writeups de las máquinas resueltas en HackTheBox 
y OffSec Labs
